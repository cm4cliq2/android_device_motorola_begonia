...
# fix, avoid rm /sdcard files
if ! grep sdcard /proc/mounts
then
	rm -f /sdcard
	mkdir /sdcard
fi
...
usleep 50000
mount -t yaffs2 -o rw,noatime,nodiratime /dev/block/mtdblock7 /system

# retry without type & options if not mounted
[ ! -f /system/build.prop ] && mount -o rw /dev/block/mtdblock7 /system
...
[ ! -f /system/build.prop ] && mount -t yaffs2 -o rw,noatime,nodiratime,errors=continue /dev/block/mtdblock7 /system
